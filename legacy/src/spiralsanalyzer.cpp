...
template<> SpiralsAnalyzer<dpoint>::SpiralsAnalyzer() : AreaAnalyzer<dpoint>() {}
...
/*template<> ListBase<dpoint> SpiralsAnalyzer<dpoint>::getArea(qreal limit,QPointF START,qreal preceding){
...
template<> ListBase<dpoint> SpiralsAnalyzer<dpoint>::getFirstArea(qreal limit){
    dpoint result = dpoint();
    // needs to be a COPY
    CliffsAnalyzer<dpoint> calculator = *this;
    ListBase<dpoint> temp = ListBase<dpoint>();
    ...
    if (calculator.size()>1){
        ...
        SpiralsAnalyzer<dpoint> legalSegment = temp;//SpiralsAnalyzer<dpoint>(temp);
        //legalSegment = SpiralsAnalyzer<dpoint>(calculator.getFirstLegalSegment());
        ...
        //SpiralsAnalyzer<dpoint> preceding = temp;
        //SpiralsAnalyzer<dpoint> preceding = legalSegment.getArea(limit,legalSegment.first());
            ...
            if (calculator.size()>1){
                ...
                if (legalSegment.size()>0){
                    ...
                    while (calculator.size()>1){
                        ...
                        sumOfAllPreceding += preceding.sumOfDxAreasRotY() + CliffsCalculator<dpoint>::area(preceding.lengthFromStartToEnd(),limit);
